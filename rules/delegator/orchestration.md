...
## Integration with Automated Workflow System

When operating in **automated mode** (workflow-orchestrator or ralph-improver agents), delegation follows the same rules but with additional context.

### Automated Mode Detection

Check for `.claude/agent-state.json`:
```bash
if [[ -f ".claude/agent-state.json" ]]; then
  # Automated mode active
  # Include workflow context in delegations
fi
```

### Enhanced Delegation Context

In automated mode, include:
```markdown
TASK: [original task]
...
- Current goal: [specific step]

PREVIOUS ATTEMPTS: [if retry]
- Attempt 1: [what was tried, outcome]
- Attempt 2: [what was tried, outcome]

[Rest of 7-section format...]
```

### Auto-Delegation Rules

In automated mode, agents can auto-delegate without asking when:
- Architecture decision detected
- Security concern found
- After 2+ failed attempts
- Complex design question

Notify user with timestamp:
```
[12:30] Delegating to Architect...
```

### Learning from Expert Feedback

After receiving expert recommendation:
1. Record in learnings database
...
```json
{
  ...
  "trigger": "Architecture decision: caching strategy",
  "expert": "architect",
  "recommendation": "Use Redis for distributed caching",
  "outcome": "Implemented successfully",
  "pattern": "Redis for multi-instance caching"
}
```

### Compact After Delegation

After expert consultation:
1. Synthesize recommendation
2. Apply changes
3. Verify
4. Compact context (save expert insights)

This prevents context bloat from expert responses.
